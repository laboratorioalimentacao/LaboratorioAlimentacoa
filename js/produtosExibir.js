...
function visualQuantia() {
    const divQuantia = document.querySelector(".quantia");
    if (!divQuantia) return; // evita erro se não existir

    const carrinho = JSON.parse(localStorage.getItem("carrinho")) || [];
    const quantia = carrinho.length;

    if (quantia <= 0) {
        divQuantia.style.display = "none";
    } else {
        divQuantia.style.display = "flex";
        divQuantia.textContent = quantia;
    }
}
